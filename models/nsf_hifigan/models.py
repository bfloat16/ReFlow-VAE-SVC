...
class SineGen(torch.nn.Module):
    ...
    def _f02sine(self, f0_values, upp):
        ...
        diff = F.conv2d(tmp_over_one.unsqueeze(1), torch.FloatTensor([[[[-1.], [1.]]]]).to(tmp_over_one.device), stride=(1, 1), padding=0, dilation=(1, 1)).squeeze(1)  # Equivalent to torch.diff, but able to export ONNX
        cumsum_shift = (diff < 0).double()
        cumsum_shift = torch.cat((torch.zeros((f0_values.size()[0], 1, self.dim), dtype=torch.double).to(f0_values.device), cumsum_shift), dim=1)
...
class Generator(torch.nn.Module):
    ...
    def remove_weight_norm(self):
        ...
        if _OLD_WEIGHT_NORM:
            ...
            torch.nn.utils.parametrize.remove_parametrizations(self.conv_pre, 'weight')
            torch.nn.utils.parametrize.remove_parametrizations(self.conv_post, 'weight')
